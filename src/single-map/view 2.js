...
document.addEventListener("DOMContentLoaded", () => {
    const mapContainers = document.querySelectorAll(".wp-block-muotamap-single-map");
...
async function erzeugePopupContent(markerData) {
	const { latitude, longitude, title, text, image, linkedContent } = markerData;
...
function route_anzeigen(map, ampelmannButton) {
	...
	mapContainer.classList.add('has_active_route');
	mapContainer.querySelectorAll('.button.single_marker').forEach(el => el.classList.remove('active'));
	ampelmannButton.classList.add('active');
