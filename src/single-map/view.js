...
const styleURL =
	"https://api.mapbox.com/styles/v1/ulrichdigital/cm42iduyt00x101r20fa19tlr/tiles/512/{z}/{x}/{y}?access_token=pk.eyJ1IjoidWxyaWNoZGlnaXRhbCIsImEiOiJjbTQyaTk1ZjMwMHc1MmpxdzNwb2gzcnJ5In0.dNCkyJW5SkOCLz0_hGLMxA";
...
document.addEventListener("DOMContentLoaded", () => {
	...
	function waitForRouting(callback) {
	...
	mapContainers.forEach((mapElement) => {
		...
		if (isBackend) {
			...
			if (mapElement._leaflet_id || mapElement._mapInstance) {
				return; // Karte schon initialisiert
			}
			...
			for (const markerData of markers) {
				...
				map.on("popupopen", (e) => {
					...
					if (mapContainer && popupContainer) {
						...
						const existingClonedPopups =
							mapContainer.querySelectorAll(
								".cloned-leaflet-popup",
							);
						...
						const elementsWithStyle =
							clonedPopup.querySelectorAll("[style]");
				...
				marker.on("click", function (event) {
					...
					erzeugePopupContent(markerData)
						.then((popupContent) => {
						...
						.catch((err) => {
							console.error(
								"Fehler beim Generieren des Popup-Inhalts:",
								err,
							);
						});
				});
			...
			mapElement.addEventListener("click", (e) => {
				...
				const singleMarkerButton = e.target.closest(
					".button.single_marker",
				);
			...
			const CustomControl = L.Control.extend({
				...
				onAdd: function (map) {
					...
					const container = L.DomUtil.create(
						"div",
						"leaflet-bar custom-control leaflet-control-map_reset_container",
					);
			...
			map.on("click", function (event) {
    			...
    			if (domTarget.classList.contains('leaflet-map-pane')) {
					console.log("reine map geklickt");
					popups_schliessen(mapElement);
					// Aktive Marker entfernen
...
async function erzeugePopupContent(markerData) {
	const { latitude, longitude, title, text, image, linkedContent } =
		markerData;
...
function alle_aktiven_marker_entfernen(mapElement) {
	const aktiveMarker = mapElement.querySelectorAll(
		".leaflet-marker-icon .active",
	); // Alle aktiven Marker finden
...
function route_anzeigen(map, ampelmannButton) {
	const mapContainer = map.getContainer();

	mapContainer.classList.add("has_active_route");
	mapContainer
		.querySelectorAll(".button.single_marker")
	...
	const routingControl = L.Routing.control({
		...
		serviceUrl: "http://router.project-osrm.org/route/v1",
		lineOptions: {
			styles: [{ color: "#964226", weight: 8, opacity: 0.8 }],
			addWaypoints: false,
		},
		createMarker: () => null,
	});

	map._routeControl = routingControl.addTo(map); // ✅
}
...
function ampelmann_anzeigen(container) {
	...
	container._ampelmannMarker = L.marker(currentCoordinates, {
		icon: ampelmannIcon,
	}).addTo(container);
...
function aktualisiereAmpelmannMarker(mapElement) {
	if (mapElement._ampelmannMarker) {
		if (currentCoordinates[1] < previousCoordinates[1]) {
			mapElement._ampelmannMarker
				.getElement()
				.classList.add("nase_links");
		} else {
			mapElement._ampelmannMarker
				.getElement()
				.classList.remove("nase_links");
		}
