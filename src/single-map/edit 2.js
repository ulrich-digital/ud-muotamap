...
//const styleURL = 'https://api.mapbox.com/styles/v1/ulrichdigital/cm42iduyt00x101r20fa19tlr/tiles/512/{z}/{x}/{y}?access_token=pk.eyJ1IjoidWxyaWNoZGlnaXRhbCIsImEiOiJjbTQ0MWhiOGIwamVpMmtzaXJiOTFyczB3In0.ZDVUjA8Rf0e0iPAsKeyd_A';
...
registerBlockType('muotamap/single-map', {
	...
	edit: ({ attributes, setAttributes, clientId }) => {
		...
		useEffect(() => {
			if (!mapRef.current && mapContainerRef.current) {
				...
				markers.forEach(({ latitude, longitude, title }, index) => {
    				...
    				mapMarkersRef.current[index] = marker; // Speichere die Marker-Referenz
		...
		useEffect(() => {
			if (mapRef.current) {
				// Registriere den `popupopen`-Handler
				mapRef.current.on("popupopen", (e) => handlePopupOpen(mapRef.current, e));

				// Bereinige den Event-Handler beim Verlassen
				return () => {
					mapRef.current.off("popupopen", (e) => handlePopupOpen(mapRef.current, e));
				};
			}
		}, [markers]); // Reagiere auf Änderungen bei Markern und Modal-Zustand
		...
		useEffect(() => {
			if (mapRef.current) {
				if (isModalOpen) {
					// Entferne die Control, falls sie existiert
					if (addMarkerControlRef.current) {
						mapRef.current.removeControl(addMarkerControlRef.current);
						addMarkerControlRef.current = null;
					}
				} else {
					// Füge die Control hinzu, falls sie nicht existiert
					if (!addMarkerControlRef.current) {
						const AddMarkerControl = L.Control.extend({
							options: { position: 'bottomright' },
							onAdd: function (map) {
								const container = L.DomUtil.create('div', 'custom-control leaflet-control-add-marker');
								const button = L.DomUtil.create('a', 'custom-control-button', container);
								button.title = 'Marker hinzufügen';
								button.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path d="M215.7 499.2C267 435 384 279.4 384 192C384 86 298 0 192 0S0 86 0 192c0 87.4 117 243 168.3 307.2c12.3 15.3 35.1 15.3 47.4 0zM168 280l0-64-64 0c-13.3 0-24-10.7-24-24s10.7-24 24-24l64 0 0-64c0-13.3 10.7-24 24-24s24 10.7 24 24l0 64 64 0c13.3 0 24 10.7 24 24s-10.7 24-24 24l-64 0 0 64c0 13.3-10.7 24-24 24s-24-10.7-24-24z"/></svg> Marker hinzufügen`;

								L.DomEvent.on(button, 'click', function (e) {
									L.DomEvent.stopPropagation(e);
									L.DomEvent.preventDefault(e);
									const mapCenter = map.getCenter();
									// Neuer Marker wird erstellt
									const newMarker = {
						...
						const control = new AddMarkerControl();
						mapRef.current.addControl(control);
						addMarkerControlRef.current = control;
		...
		\* =============================================================== */
		useEffect(() => {
			const handleEditAndDeleteButtonClick = (event) => {
				const domTarget = event.originalEvent?.target;

				// Edit-Button
				const editButton = domTarget?.closest('.cloned-leaflet-popup .button.edit');
				if (editButton) {
					const popup = editButton.closest('.cloned-leaflet-popup');
					if (popup) {
						const markerIndex = markers.findIndex((marker) =>
							popup.innerHTML.includes(marker.title || `Marker ${index + 1}`)
						);
						if (markerIndex !== -1) {
							setIsModalOpen(true); // Modal öffnen
							setEditingMarkerIndex(markerIndex); // Bearbeiteten Marker setzen
						}
					}
				}

				// Delete-Button
				const deleteButton = domTarget?.closest('.cloned-leaflet-popup .button.delete');
				if (deleteButton) {
					const popup = deleteButton.closest('.cloned-leaflet-popup');
					if (popup) {
						const markerIndex = markers.findIndex((marker) =>
							popup.innerHTML.includes(marker.title || `Marker ${index + 1}`)
						);
						if (markerIndex !== -1) {
							setIsDeleteModalOpen(true); // Delete-Modal öffnen
							setMarkerToDeleteIndex(markerIndex); // Zu löschenden Marker setzen
						}
					}
				}
			};

			mapRef.current.on('click', handleEditAndDeleteButtonClick);

			return () => {
				mapRef.current.off('click', handleEditAndDeleteButtonClick);
			};
		}, [markers]);
...
useEffect(() => {
        const handleClickOnBlock = (event) => {
            ...
            // Prüfen, ob der Klick auf das aktuelle Block-Element erfolgt
            const parentMapContainer = domTarget.closest('.wp-block-muotamap-single-map');
            if (parentMapContainer) {
                // Block manuell als ausgewählt markieren
		...
		useEffect(() => {
			const removeClonedPopupAndCleanMarkers = (event) => {
				const domTarget = event.originalEvent?.target; // Zugriff auf das DOM-Element
				// Prüfen, ob das Ziel kein Marker, Button oder Popup ist
				if (!domTarget.closest('.leaflet-marker-icon') && // Kein Marker
					!domTarget.closest('.button')) { // Kein Button
					
					// Suche nach dem nächsten Elterncontainer `.wp-block-muotamap-single-map`
					const parentMapContainer = domTarget.closest('.wp-block-muotamap-single-map');
					if (parentMapContainer) {
						const existingClonedPopups = parentMapContainer.querySelectorAll('.cloned-leaflet-popup');
						if (existingClonedPopups.length > 0) {
							existingClonedPopups.forEach((popup) => {
								popup.remove(); // Entferne jedes gefundene Popup
							});
						} 
					} 
				}
			...
			// Event-Listener für die Karte hinzufügen
