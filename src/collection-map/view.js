...
function isRoutingAvailable() {
	return typeof L !== 'undefined' && L.Routing && typeof L.Routing.control === 'function';
}
    ...
    mapContainers.forEach((mapElement) => {
        ...
        if (isBackend) {
			...
			function initializeCollectiveMap(mapElement) {
  				if (mapElement._leaflet_id || mapElement._mapInstance) return; // ✅ already inited
...
waitForRouting(() => {
		...
		ampelmann_anzeigen(mapElement._mapInstance);
			...
			*/
			initializeCollectiveMap(mapElement);
