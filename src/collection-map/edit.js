...
// Block Definition
registerBlockType('muotamap/collection-map', {
	title: 'Collection Map',
	icon: 'location-alt',
	category: 'muotamaps',
	title: 'Collection Map',
	icon: 'location-alt',
	description: 'Dieser Block zeigt eine Karte mit allen Markern aus den Single-Maps an. Die verlinkten Magazinbeiträge, Texte und Bilder werden übernommen.',
...
// (A) Map initialisieren + Marker laden
useEffect(() => {
  async function fetchMarkers() {
    try {
      const response = await fetch('/wp-json/custom/v1/all-pages-markers');
      if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
      const data = await response.json();
      setAttributes({ markers: data });
    } catch (error) {
  ...
  if (!mapRef.current && mapContainerRef.current) {
    ...
    // erst nach Map-Init laden
    fetchMarkers();
  }
}, [latitude, longitude, zoom, mapContainerRef]);

// (B) Marker rendern – nur wenn Map + Icon + Daten da sind
useEffect(() => {
  const map = mapRef.current;
  if (!map || !customIcon) return;
...
useEffect(() => {
	...
	const handleClickOnBlock = (event) => {
		...
		const parentMapContainer = domTarget.closest('.wp-block-muotamap-collection-map');
	...
	if (!mapRef.current) return;
			...
			const removeClonedPopupAndCleanMarkers = (event) => {
				...
				if (!domTarget.closest('.leaflet-marker-icon') && // Kein Marker
					...
					// Suche nach dem nächsten Elterncontainer `.wp-block-muotamap-collection-map`
					const parentMapContainer = domTarget.closest('.wp-block-muotamap-collection-map');
					if (parentMapContainer) {
...
**/}
	...
	save: ({ attributes }) => {
		...
		const blockProps = useBlockProps.save({
			className: 'wp-block-muotamap wp-block-muotamap-collection-map alignwide',
